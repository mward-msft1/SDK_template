...
3. **Agent 365 SDK** reporting hooks
...
4. Keep Purview app registration IDs aligned between:
...
5. Register the middleware from `agentMiddlewareTemplate.js` in your host runtime and route each user turn through it.
6. If you prefer Python, start from `template/python/example_runner.py`.
7. If you prefer C++, start from `template/cpp/src/example_runner.cpp`.
8. If you prefer C#/.NET, start from `template/dotnet/Program.cs`.
9. If you prefer Rust, start from `template/rust/src/main.rs`.
10. If you prefer Go, start from `template/go/main.go`.
